...
class WatercolorCanvas extends Component {
  ...
  addDrip(scene, point) {
    ...
    var drip = new Drip({
      ...
      position: point,
      velocity: new Vector2D(Math.random() * 2, Math.random() * 2),
      color:
        Math.random() < 0.2
          ? '#fafafa'
          : colors[selectedColors][
              Pollock.randomInRange(0, colors[selectedColors].length - 1)
            ],
  ...
  renderColor(color, idx) {
    return (
      <div
        className="WatercolorCanvas__colors__block__color"
